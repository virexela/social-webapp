...
function sha256Js(input: Uint8Array): Uint8Array {
...
export async function sha256(data: Uint8Array): Promise<Uint8Array> {
  const safeInput = new Uint8Array(data);
  const subtle = globalThis.crypto?.subtle;
  if (subtle?.digest) {
    const digest = await subtle.digest("SHA-256", safeInput.buffer);
    return new Uint8Array(digest);
  }
  return sha256Js(safeInput);
}
